...
代码层面应先支持以下 broad candidate set：RSAA、RCSAA、$U/C$ modified-$\chi^2$ DRO、$U/C$ $W_1$-DRO 以及 $U/C$ PCM-DRO，并保留 selected nominal CSAA 作为 contextual nominal reference。最终论文主表\textbf{不在 Experiment 1 结果出来之前强行剪枝}：
\begin{itemize}
\item 若 Experiment 1 显示 contextual weighting 相对 SAA-All / Tuned-SAA 有清晰且稳定的优势，则主文可简化为 selected CSAA、RCSAA、$C$-$\chi^2$、$C$-$W_1$ 与 $C$-PCM，必要时仅保留 $U$-$W_1$ 作为一项 supplementary robustness benchmark；
\item 若 contextual gain 较弱、mixed 或不稳定，则保留 RSAA、$U$-$\chi^2$、$U$-$W_1$、$U$-PCM 等 stronger unconditional robust baselines，避免主文预设“contextualization 必然更好”。
\end{itemize}
...
\qquad
\mathcal U=\prod_{j\in\J}[0,1.5m_j].
\]
因此 $c$ 是无量纲的逐 lane 标准化平均 $L_1$ 距离，$\mathcal U$ 的 ground-metric diameter 至多为 $1.5$。对参考分布 $\widehat P_\pi=\sum_{s\in\mathcal S_{\mathrm{tr}}}\pi_s\delta_{\boldsymbol d^s}$，歧义集为
\[
...
\subsection{PCM-DRO benchmark}

Moment 基准只保留 partial cross-moment (PCM)，不使用 Mean--MAD。对于参考权重 $\pi_s$，先计算
\[
\widehat\mu_j=\sum_s\pi_s d_j^s,\qquad
\widehat v_j=\sum_s\pi_s(d_j^s-\widehat\mu_j)^2,\qquad
\widehat v_{\Sigma}=\sum_s\pi_s\left(\sum_j(d_j^s-\widehat\mu_j)\right)^2.
\]
歧义集只保留逐 lane 的边际方差和一条总需求方差约束，而非 $60\times60$ full covariance：
\[
\mathcal P_{\mathrm{PCM}}(\kappa)
=\left\{
P:\ P(\boldsymbol d\in\mathcal U)=1,\ 
\mathbb E_P[d_j]=\widehat\mu_j,\ 
\mathbb E_P[(d_j-\widehat\mu_j)^2]\le \kappa^2\widehat v_j,\ \forall j,\quad
\mathbb E_P\left[\left(\sum_j(d_j-\widehat\mu_j)\right)^2\right]
\le\kappa^2\widehat v_{\Sigma}
\right\}.
\]
其中 unconditional reference 使用 equal-weight estimates，contextual reference 使用 $\pi_s$-weighted estimates；两者只改变参考权重，不改变歧义集结构。总需求方差含跨 lane 协方差信息，但只约束其总和，不估计完整协方差矩阵。沿用本说明书的一个 global inflation factor，同时放大边际和总量方差上界：
\[
\kappa\in\{1,1.25,1.5,2\},
\]
并通过相同 rolling validation criterion 选择 $\kappa^\star$。正式求解采用 RSOME lifted affine decision rules，须明确标为策略近似，不能称为完全自适应二阶段 PCM-DRO 的精确解。当前适配器只接受 RSOME/MOSEK 的 Optimal 状态；在该状态下沿用现有输出约定 $\texttt{bestBound}=\texttt{objective}$、$\texttt{gap}=0$，其证书仅针对该 lifted-affine MISOCP 近似。Experiment 2 runner 已实现上述网格、U/C 两种矩估计和统一 OOS 评价；正式规模尚待运行。
...
Wasserstein geometry & training-max-scaled average-lane $L_1$; box $[0,1.5m_j]$ & exact definition in Exp.~2\\
Wasserstein radius & $\{.005,.01,.025,.05,.1,.25,.5,1\}$ & absolute $\varepsilon$, validation-selected\\
PCM $\kappa$ & $\{1,1.25,1.5,2\}$ & Exp.~2 only; marginal and total-demand bounds\\

...
\]
因此 $c$ 是无量纲的逐 lane 标准化平均 $L_1$ 距离，$\mathcal U$ 的 ground-metric diameter 至多为 $1$。该box同时用于Wasserstein和PCM，使连续支撑型基准均限制在逐lane训练经验范围内。对参考分布 $\widehat P_\pi=\sum_{s\in\mathcal S_{\mathrm{tr}}}\pi_s\delta_{\boldsymbol d^s}$，歧义集为
\[

...
TEST_ROOT = OUT_ROOT / "08_额外测试_极小权重替代与大lambda扩展" / "02_保持当前代码_扩展lambda到300_500_1000"
...
LAMBDA_ORDER = ["0", "0.01", "0.05", "0.1", "1", "5", "10", "50", "100", "300", "500", "1000"]
...
def plot_combined(rows: list[dict[str, object]]) -> None:
    ...
    for ax, (metric, ylabel) in zip(axes, specs):
        ...
        ax.legend(handles, labels, frameon=False, loc="lower right", handlelength=2.6)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "ext_lambda_combined.pdf", bbox_inches="tight")
    fig.savefig(FIG_DIR / "ext_lambda_combined.png", dpi=600, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    write_csv(EXTRACT_DIR / "k=1_扩展lambda到300_500_1000_summary.csv", rows, fieldnames)
    plot_single(rows, "imp_mean", r"$\Delta_{\mathrm{cost}}$ (\%)", "ext_lambda_mean")
    plot_single(rows, "imp_std", r"$\Delta_{\mathrm{std}}$ (\%)", "ext_lambda_std")
    plot_single(rows, "imp_max", r"$\Delta_{\max}$ (\%)", "ext_lambda_max")
    plot_single(rows, "mean_selected_count", "ANC", "ext_lambda_selected")
    plot_combined(rows)
    print(f"prepared test2 outputs under {TEST_ROOT}")

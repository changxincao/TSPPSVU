...
$testRoot = 'D:\软件\eclipse\workspace\TransportationProcurement\analysis\巴西数据分析\新版_purchase时间\输出\08_额外测试_极小权重替代与大lambda扩展\01_极小权重替代为1e-6并重标化_仅测试'
...
$lambdas = @('0.01', '0.05', '0.10', '1.00', '5.00', '10.00', '50.00', '100.00')
$jobs = @()
...
foreach ($lambda in $lambdas) {
  $outDir = Join-Path $rawRoot ('RCSAA_W50_k1=1_raw_C0.10_lambda{0}_tinyFloor1e-6_renorm' -f $lambda)
  if (Has-GlobalSummary $outDir) {
    Append-State ('SKIP existing lambda={0}' -f $lambda)
    continue
  }
  $jobs += [pscustomobject]@{
    Name = ('test1_k1=1_C=0.10_lambda={0}' -f $lambda)
    Args = @(
      '-Dtp.dro.replaceTinyWeights=true',
      '-Dtp.dro.tinyWeightFloor=1e-6',
      '-cp', $javaCp,
    ...
    StdOut = (Join-Path $logRoot ('log_test1_k1=1_C0.10_L{0}_out.txt' -f $lambda))
    StdErr = (Join-Path $logRoot ('log_test1_k1=1_C0.10_L{0}_err.txt' -f $lambda))
  }
}

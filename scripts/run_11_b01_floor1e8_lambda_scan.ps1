...
$expRoot = 'D:\杞欢\eclipse\workspace\TransportationProcurement\analysis\宸磋タ鏁版嵁鍒嗘瀽\鏂扮増_purchase鏃堕棿\杈撳嚭\11_B=0.1_鏉冮噸涓嬮檺1e-8閲嶈窇_lambda鍏ㄦ壂鎻?
$rawRoot = Join-Path $expRoot '01_raw_outputs'
$processedRoot = Join-Path $expRoot '02_processed'
$figRoot = Join-Path $expRoot '03_figures'
$logRoot = Join-Path $expRoot 'runlog'
$stateFile = Join-Path $logRoot 'state.txt'

New-Item -ItemType Directory -Force -Path $rawRoot | Out-Null
New-Item -ItemType Directory -Force -Path $processedRoot | Out-Null
New-Item -ItemType Directory -Force -Path $figRoot | Out-Null
...
Append-State 'formalLogic=floor weights <= 1e-8 to 1e-8, then renormalize; do not drop scenarios'
...
foreach ($lambda in $lambdas) {
  ...
  $jobs += [pscustomobject]@{
    ...
    StdOut = (Join-Path $logRoot ('log_k1=1_C{0}_L{1}_out.txt' -f $c, $lambda))
    StdErr = (Join-Path $logRoot ('log_k1=1_C{0}_L{1}_err.txt' -f $c, $lambda))
  }
}
